...
import { terminerCourseExterne } from '@/src/actions/courses.actions';

const DeliveryValidate = ({ restaurant, delivery, open, setOpen }: { restaurant: Restaurant; delivery: CourseExterne; open: boolean; setOpen: (open: boolean) => void }) => {
    const [pending, setPending] = useState(false);

    const router = useRouter();
    const handleSubmit = async () => {
        const result = await terminerCourseExterne(delivery.id);
                        ...
                        >
                            <DialogPanel className="panel w-full max-w-lg overflow-hidden rounded-lg border-0 p-0 text-black dark:text-white-dark">
                                ...
                                <div className="grid gap-4 p-5">
                                    <p className="text-gray-500 dark:text-gray-400">Voulez-vous terminer la course ?</p>
