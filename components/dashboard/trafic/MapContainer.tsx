...
const DEFAULT_AVATAR_ICON = '/assets/images/avatar.png'; // Chemin vers une icône par défaut disponible dans votre projet public/

const createMarkerIcon = async (imageUrl: string, name: string, isDark: boolean): Promise<string> => {
  ...
  return new Promise<string>((resolve) => {
    ...
    img.onerror = () => {
      // En cas d'erreur, résoudre avec l'icône par défaut
...
export default function MapContainer({ couriers, selectedCourierId, onMarkerClick }: MapContainerProps) {
  ...
  // Memoisation des composants MarkerAsync
    ...
    return couriers.map((courier) => {
      ...
      return (
        <MarkerAsync
          ...
          animation={
            selectedCourierId === courier.livreurId ? google.maps.Animation.BOUNCE : undefined
          }
        />
      );
    });
  ...
  if (!isLoaded)
    return <div className="p-4 dark:text-gray-300">Chargement de la carte...</div>;
...
const MarkerAsync = ({ iconPromise, ...props }: MarkerAsyncProps) => {
  ...
  useEffect(() => {
    ...
    iconPromise.then((url) => {
      if (isMounted) {
        // Si url est vide, on utilise l'avatar par défaut
        setIconUrl(url || DEFAULT_AVATAR_ICON);
      }
    });
    return () => {
      isMounted = false;
