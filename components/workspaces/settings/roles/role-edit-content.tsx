...
import { updateRole } from '@/src/actions/roles.actions';
...
export default function RoleEditContent({ role, modules }: { role: Role; modules: Module[] }) {
    const router = useRouter();
    // Initialize state with existing role data
    const [selectedModuleId, setSelectedModuleId] = useState(modules[0]?.id);
    const [roleName, setRoleName] = useState(role.name);
    const [selectedPermissions, setSelectedPermissions] = useState<SelectedPermissions>({});

    // Initialize selected permissions based on existing role permissions
    useEffect(() => {
        const initialPermissions: SelectedPermissions = {};

        // Group existing role actions by module
        const roleActionsByModule = role?.role_actions?.reduce(
            (acc, { action }) => {
                const moduleId = action.module.id;
                if (!acc[moduleId]) {
                    acc[moduleId] = [];
                }
                acc[moduleId].push(action.id);
                return acc;
            },
        ...
        modules.forEach((module) => {
            ...
            const selectedModuleActions = roleActionsByModule![module.id] || [];

            initialPermissions[module.id] = {
                all: selectedModuleActions.length === moduleActions.length,
    ...
    const [, formAction] = useFormState(
        async (prevState: any, _formData: FormData) => {
            ...
            const result = await updateRole(formData, role.id);
        ...
        <DrawerContent>
            ...
            <div className="flex h-[calc(100vh-4rem)] relative gap-6 px-4">
                    ...
                    <div className="flex-1">
                        <div className="mb-2">
                            <h2 className={title({ size: 'h3', className: 'text-primary' })}>Modifier un rôle</h2>
                            <p className="text-sm text-gray-500">Modifiez les permissions du rôle {role.name}.</p>
                        </div>
                        ...
                        <div className="flex w-full items-center gap-2 py-2">
                            ...
                            <SubmitButton className="w-fit">Mettre à jour</SubmitButton>
                        </div>
                    </div>
                </div>
            </form>
