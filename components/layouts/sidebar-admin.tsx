                ...
                <div className="h-full bg-white dark:bg-black">
                    ...
                    <PerfectScrollbar className="relative h-[calc(100vh-80px)]">
                        <ul className="relative space-y-0.5 p-4 py-0 font-semibold">
                            <RenderMenu menu={menuDataAdmin} currentMenu={currentMenu} toggleMenu={toggleMenu} t={t} />
                        </ul>
                    </PerfectScrollbar>
                </div>
            </nav>
...
function RenderMenu({ menu, currentMenu, toggleMenu, t }: { menu: IMenuDataAdmin[]; currentMenu: string; toggleMenu: (value: string) => void; t: (value: string) => string }) {
    const renderMenuItem = (item: IMenuDataAdmin, key: number) => {
        return (
    ...
    const renderItem = (item: IMenuDataAdmin, key: number) => {
        return (
            <li key={key} className="nav-item">
                <Link href={`/lunion-booking-admin${item.path ?? ''}`} className="group">
                    <div className="flex items-center">
                        {item.icon && <item.icon className="shrink-0 group-hover:!text-primary" />}
    ...
    const renderItemMenuHeader = (item: IMenuDataAdmin, key: number) => {
        return (
            <Fragment key={key}>
