...
const Header = ({ reference, profileOrganisations }: { reference: string; profileOrganisations: ProfileOrganisations | null }) => {
            ...
            <header className={`z-40 ${themeConfig.semidark && themeConfig.menu === 'horizontal' ? 'dark' : ''}`}>
                <div className="shadow-sm">
                    ...
                    <ul className="horizontal-menu hidden border-t border-[#ebedf2] bg-white px-6 py-1.5 font-semibold text-black rtl:space-x-reverse dark:border-[#191e3a] dark:bg-black dark:text-white-dark lg:space-x-1.5 xl:space-x-8">
                        <RenderMenu reference={reference} menu={menuData} t={t} />
                    </ul>
                </div>
            </header>
        </>
...
function RenderMenu({ reference, menu, t }: { reference: string; menu: IMenuData[]; t: (value: string) => string }) {
    ...
    const renderItem = (item: IMenuData, key: number) => {
        return (
            <Link href={`/workspaces/${reference}${item.path ?? ''}`} key={key} type="button" className="nav-link">
