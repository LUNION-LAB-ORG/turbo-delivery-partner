...
function SortableListItem({ item, order, onCompleteItem, onRemoveItem, renderExtra, handleDrag, isExpanded, className }: SortableListItemProps) {
        ...
        <motion.div className={cn('', className)} key={item.id}>
            <div className="flex w-full items-center">
                ...
                >
                            ...
                            <AnimatePresence>
                                {!isExpanded ? (
                                    ...
                                    >
                                        ...
                                        >
                                            <h4 className={cn('tracking-tighter text-base md:text-lg ', item.checked ? 'text-red-400' : 'text-white/70')}>
                    ...
                    <div onPointerDown={isDraggable ? handleDragStart : undefined} style={{ touchAction: 'none' }} />
                </Reorder.Item>
        ...
        return (
            <LayoutGroup>
                <Reorder.Group axis="y" values={items} onReorder={setItems} className="flex flex-col">
                    <AnimatePresence>
                        {items?.map((item, index) => renderItem(item, index, onCompleteItem, (id: number) => setItems((items) => items.filter((item) => item.id !== id))))}
                    </AnimatePresence>
                </Reorder.Group>
            </LayoutGroup>
        );
